  ...
  describe('template', () => {
    describe('basic functionality', () => {
      it('should replace simple placeholders', () => {
        ...
        expect(
          template('Hi {firstName} {lastName}', {
            firstName: 'John',
            lastName: 'Doe',
          }),
        ).toBe('Hi John Doe')
      ...
      it('should handle nested object properties', () => {
        ...
        expect(template('User: {user.name} ({user.id})', data)).toBe(
          'User: John (123)',
        )
        expect(template('Theme: {config.theme} v{config.version}', data)).toBe(
          'Theme: dark v1.0.0',
        )
      })
      ...
      it('should handle array access', () => {
        ...
        expect(template('First user: {users.0.name}', data)).toBe(
          'First user: John',
        )
        expect(template('Second user: {users.1.name}', data)).toBe(
          'Second user: Jane',
        )
        expect(template('First color: {colors.0}', data)).toBe(
          'First color: red',
        )
      })
    ...
    describe('edge cases', () => {
      it('should handle missing keys with fallback', () => {
        expect(
          template('Hello {missing}', {}, { fallback: '[not found]' }),
        ).toBe('Hello [not found]')
        expect(template('Hi {name}', {}, { fallback: 'Guest' })).toBe(
          'Hi Guest',
        )
        expect(template('Count: {count}', {}, { fallback: '0' })).toBe(
          'Count: 0',
        )
      })
      ...
      it('should handle null and undefined values', () => {
        ...
        expect(template('Undefined: {undefinedValue}', data)).toBe(
          'Undefined: ',
        )
        expect(template('Empty: {emptyString}', data)).toBe('Empty: ')

        // With fallback
        expect(template('Null: {nullValue}', data, { fallback: 'N/A' })).toBe(
          'Null: N/A',
        )
        expect(
          template('Undefined: {undefinedValue}', data, { fallback: 'N/A' }),
        ).toBe('Undefined: N/A')
        expect(
          template('Empty: {emptyString}', data, { fallback: 'N/A' }),
        ).toBe('Empty: ')
      })
      ...
      it('should handle templates with no placeholders', () => {
        ...
        expect(template('No placeholders here!', {})).toBe(
      ...
      it.skip('should handle nested placeholders (should not evaluate)', () => {
        expect(template('Hello {{name}}', { name: 'World' })).toBe(
          'Hello {World}',
        )
        expect(template('Value: {{{count}}}', { count: 42 })).toBe(
          'Value: {42}',
        )
      })
    })
    ...
    describe('options', () => {
      describe('prefix and suffix', () => {
        it('should add prefix and suffix to values', () => {
          expect(
            template('Price: {price}', { price: '99.99' }, { prefix: '$' }),
          ).toBe('Price: $99.99')
          expect(
            template('Name: {name}', { name: 'John' }, { suffix: '!' }),
          ).toBe('Name: John!')
          expect(
            template(
              'Value: {value}',
        ...
        it('should not add prefix/suffix to fallback values', () => {
          expect(
            template('Price: {missing}', {}, { prefix: '$', fallback: 'N/A' }),
          ).toBe('Price: N/A')
          expect(
            template(
              'Name: {missing}',
              {},
              { suffix: '!', fallback: 'Unknown' },
            ),
          ).toBe('Name: Unknown')
        })
      })
      ...
      describe('escapeHtml', () => {
        it('should escape HTML when enabled', () => {
          ...
          expect(template('Content: {html}', data, { escapeHtml: true })).toBe(
            'Content: &lt;script&gt;alert(&quot;xss&quot;)&lt;/script&gt;',
          )
          expect(template('Text: {html}', data, { escapeHtml: false })).toBe(
            'Text: <script>alert("xss")</script>',
          )
          expect(template('Text: {html}', data)).toBe(
            'Text: <script>alert("xss")</script>',
          )
        })
        ...
        it('should escape various HTML characters', () => {
          ...
          expect(template('{text}', data, { escapeHtml: true })).toBe(
            'Hello &amp; &quot;World&quot; &lt;tag&gt;',
          )
        })
        ...
        it('should not escape fallback values', () => {
          expect(
            template(
              '{missing}',
              {},
              { escapeHtml: true, fallback: '<default>' },
            ),
          ).toBe('<default>')
        })
      })
      ...
      describe('combined options', () => {
        it('should apply all options together', () => {
          ...
          expect(template('Price: {price}', data, options)).toBe(
            'Price: $99.99 USD',
          )
          expect(template('HTML: {html}', data, options)).toBe(
            'HTML: $&lt;b&gt;Bold&lt;/b&gt; USD',
          )
          expect(template('Missing: {missing}', data, options)).toBe(
            'Missing: N/A',
          )
        })
      })
    ...
    describe('complex nested data', () => {
      it('should handle deeply nested objects', () => {
        ...
        expect(
          template('Deep: {level1.level2.level3.level4.value}', data),
        ).toBe('Deep: deep value')
      })
      ...
      it('should handle mixed arrays and objects', () => {
        ...
        expect(template('Dept: {company.departments.0.name}', data)).toBe(
          'Dept: Engineering',
        )
        expect(
          template('Employee: {company.departments.0.employees.0.name}', data),
        ).toBe('Employee: John')
        expect(
          template('Role: {company.departments.0.employees.1.role}', data),
        ).toBe('Role: Designer')
      })
    ...
    describe('real-world examples', () => {
      it('should handle email templates', () => {
        ...
        const result = template(emailTemplate, data)
      ...
      it('should handle configuration templates', () => {
        ...
        const configTemplate =
          'Server: {server.host}:{server.port}, DB: {database.url}, Auth: {features.auth}'
        const result = template(configTemplate, config)

        expect(result).toBe(
          'Server: localhost:3000, DB: mongodb://localhost:27017/myapp, Auth: true',
        )
      })
      ...
      it('should handle internationalization templates', () => {
        ...
        expect(template('{messages.welcome} {user.name}!', i18n)).toBe(
          'Welcome María!',
        )
        expect(template('{messages.goodbye} {user.name}!', i18n)).toBe(
          'Goodbye María!',
        )
      })
    })
  ...
  describe('templateAdvanced', () => {
    describe('conditional logic', () => {
      it('should handle ternary-like conditionals', () => {
        expect(
          templateAdvanced('Hello {user.name ? user.name : "Guest"}', {
            user: { name: 'John' },
          }),
        ).toBe('Hello John')
        expect(
        ...
        expect(
          templateAdvanced('Hello {missing ? "Found" : "Missing"}', {}),
        ).toBe('Hello Missing')
      })

      it('should handle equality checks', () => {
        expect(
          templateAdvanced('{count} item{count === 1 ? "" : "s"}', {
            count: 1,
          }),
        ).toBe('1 item')
        expect(
      ...
      it('should handle inequality checks', () => {
        expect(
          templateAdvanced(
            'Role: {role !== "admin" ? "User" : "Administrator"}',
            { role: 'user' },
          ),
        ).toBe('Role: User')
        expect(
      ...
      it('should handle nested property conditions', () => {
        ...
        expect(
          templateAdvanced(
            'Access: {user.role === "admin" ? "Full" : "Limited"}',
            data,
          ),
        ).toBe('Access: Full')
        expect(
    ...
    describe('edge cases', () => {
      it('should handle missing properties in conditions', () => {
        expect(
          templateAdvanced('Hello {missing ? "Found" : "Guest"}', {}),
        ).toBe('Hello Guest')
        expect(
          templateAdvanced('Count: {missing === 1 ? "One" : "Other"}', {}),
        ).toBe('Count: Other')
      })
      ...
      it('should handle complex nested conditionals', () => {
        ...
        expect(
          templateAdvanced(
            'Edit: {user.permissions.canEdit ? "Allowed" : "Denied"}',
            data,
          ),
        ).toBe('Edit: Allowed')
      })
      ...
      it('should handle literal strings with spaces', () => {
        expect(
          templateAdvanced(
            'Message: {user.name ? user.name : "Anonymous User"}',
            {},
          ),
        ).toBe('Message: Anonymous User')
        expect(
          templateAdvanced(
            'Status: {active ? "Currently Active" : "Not Active"}',
            { active: false },
          ),
      ...
      it('should fall back to regular template for non-conditional placeholders', () => {
        expect(
          templateAdvanced('Hello {name}! {count} items.', {
            name: 'John',
            count: 5,
          }),
        ).toBe('Hello John! 5 items.')
      })
    })
    ...
    describe('mixed usage', () => {
      it('should handle both conditional and regular placeholders', () => {
        ...
        const template =
          'User: {user.name} ({user.isAdmin ? "Admin" : "User"}) - {count} item{count === 1 ? "" : "s"}'

        expect(templateAdvanced(template, data)).toBe(
          'User: John (Admin) - 3 items',
        )
      })
      ...
      it('should handle multiple conditionals in one template', () => {
        ...
        const template =
          '{count === 0 ? "No" : count} item{count === 1 ? "" : "s"} for {user.premium ? "premium" : "free"} user'
    ...
    describe('performance', () => {
      it('should handle complex templates efficiently', () => {
        ...
        const complexTemplate =
          'Status: {user.role === "admin" ? "Administrator" : "User"} with {count === 0 ? "no" : count} item{count === 1 ? "" : "s"}'
