...
describe('getBase64Image', () => {
  ...
  describe('basic functionality', () => {
    ...
    it('should return a Promise', async () => {
      // Set up the mock to immediately resolve
      ...
      // Return the promise so vitest can handle it
      return result
    })
  ...
  describe('fetch behavior', () => {
    ...
    it('should call blob() on fetch response', async () => {
      const mockBlobFn = vi.fn(async () => Promise.resolve(mockBlob))
      global.fetch = vi.fn(async () =>
        Promise.resolve({
          blob: mockBlobFn,
          ok: true,
  ...
  describe('error handling', () => {
    it('should handle fetch errors', async () => {
      ...
      await expect(
        getBase64Image('https://example.com/image.png'),
    ...
    it('should handle blob conversion errors', async () => {
      global.fetch = vi.fn(async () =>
        Promise.resolve({
          blob: async () => Promise.reject(new Error('Blob error')),
          ok: true,
          status: 200,
        }),
      )
    ...
    it('should handle FileReader errors', async () => {
      // Set up fetch mock to return a valid blob
      global.fetch = vi.fn(async () =>
        Promise.resolve({
      ...
      mockFileReader.readAsDataURL = vi.fn((blob) => {
        // Verify blob is passed correctly
    ...
    imageTypes.forEach(({ type, base64 }) => {
      it(`should handle ${type} images`, async () => {
        ...
        const result = await getBase64Image(
          `https://example.com/image.${type.split('/')[1]}`,
        )
  ...
  describe('real-world scenarios', () => {
    ...
    it('should work with relative URLs', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith('./assets/image.png', {
        mode: 'no-cors',
      })
  ...
  describe('performance considerations', () => {
    it('should handle multiple concurrent conversions', async () => {
      ...
      const promises = urls.map(async (url) => getBase64Image(url))
      const results = await Promise.all(promises)

      expect(results).toHaveLength(3)
      expect(
        results.every(
          (result) => result === 'data:image/png;base64,CONCURRENT_TEST',
        ),
      ).toBe(true)
