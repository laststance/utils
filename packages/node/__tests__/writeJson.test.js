...
describe('writeJson', () => {
  ...
  describe('object handling', () => {
    ...
    it('should handle complex objects', () => {
      ...
      const expectedContent = JSON.stringify(complexObject, null, 2).replace(/\n/g, os.EOL) + os.EOL
      expect(fs.writeFileSync).toHaveBeenCalledWith('complex.json', expectedContent)
    })
  ...
  describe('array handling', () => {
    ...
    it('should handle arrays with mixed types', () => {
      ...
      const expectedContent = JSON.stringify(expectedObject, null, 2).replace(/\n/g, os.EOL) + os.EOL
      
      expect(fs.writeFileSync).toHaveBeenCalledWith('mixed.json', expectedContent)
    })
    ...
    it('should handle nested arrays', () => {
      ...
      expect(fs.writeFileSync).toHaveBeenCalledWith('nested.json', expectedContent)
    })
  })
  ...
  describe('file naming', () => {
    it('should add .json extension to filename', () => {
      ...
      expect(fs.writeFileSync).toHaveBeenCalledWith('filename.json', expect.any(String))
      
      writeJson('another-file', data)
      expect(fs.writeFileSync).toHaveBeenCalledWith('another-file.json', expect.any(String))
    })
    ...
    it('should work with filenames containing special characters', () => {
      ...
      expect(fs.writeFileSync).toHaveBeenCalledWith('file_with_underscores.json', expect.any(String))
      
      writeJson('file-with-dashes', data)
      expect(fs.writeFileSync).toHaveBeenCalledWith('file-with-dashes.json', expect.any(String))
    })
  })
  ...
  describe('line ending handling', () => {
    it('should use platform-specific line endings for objects', () => {
      ...
      let expectedContent = JSON.stringify(testObject, null, 2).replace(/\n/g, windowsEOL) + windowsEOL
      ...
      expectedContent = JSON.stringify(testObject, null, 2).replace(/\n/g, unixEOL) + unixEOL
      expect(fs.writeFileSync).toHaveBeenCalledWith('test.json', expectedContent)
    })
